...
const FuturesVolumeVariation = () => {
  ...
  useEffect(() => {
    const loadCoins = async () => {
      try {
        setError('');
        const coinList = await fetchFuturesCoins(); // Busca moedas futuras
        setCoins(coinList);
      } catch (err) {
        console.error('Erro ao buscar moedas:', err);
        setError('Failed to fetch futures coins. Please try again.');
      }
    };
    ...
    try {
      ...
      const responses = await Promise.all(
        coins.map(async (coin) => {
          ...
          // Separar volumes em dois períodos de 15 minutos
...
export default FuturesVolumeVariation;
